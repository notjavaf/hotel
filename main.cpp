...
map <comfort, int> a = {{comfort::lux, 5},
                        ...
                        {comfort::one_seat, 5}};
vector<int> day_stat;
int M = 10, K = 25, cur_input_window = 0;
...
string s_days = "12", s_lux = "5", s_semi_lux = "5", s_two_seat_sofa = "5", s_two_seat = "5", s_one_seat = "5"; 
...
void set_param_to_static_gr_objects(){
	...
	set_param_to_text1(text_title_window_1, "Введите поочередно числа, переходя между полями с помощью стрелок", font, 50, 50, 20, sf::Color::Black);
	set_param_to_text1(text_title_window_2, "(зеленый маркер указывает на текущее поле ввода)", font, 50, 80, 18, sf::Color::Black);
...
void press_numButton(sf::Keyboard::Key keycode){
	switch (cur_input_window){
		case 0:
			if (s_days.size() < 10){
		...
		case 1:
			...
			break;
		case 2:
			if (s_semi_lux.size() < 10){
				char chr = static_cast<char>(keycode - sf::Keyboard::Num0 + '0');
                s_semi_lux.push_back(chr);
			}
			break;
		case 3:
			if (s_two_seat_sofa.size() < 10){
				char chr = static_cast<char>(keycode - sf::Keyboard::Num0 + '0');
                s_two_seat_sofa.push_back(chr);
			}
			break;
		case 4:
			if (s_two_seat.size() < 10){
				char chr = static_cast<char>(keycode - sf::Keyboard::Num0 + '0');
                s_two_seat.push_back(chr);
			}
			break;
		default:
			if (s_one_seat.size() < 10){
				char chr = static_cast<char>(keycode - sf::Keyboard::Num0 + '0');
...
void printing_in_first_window(){
	...
	print_objects(window, {text_input_days,  
		...
		switch (cur_input_window){
			case 0:
				shape.setPosition(352,152);
				break;
			case 1:
				shape.setPosition(352,252);
				break;
			case 2:
				shape.setPosition(352,302);
				break;
			case 3:
				shape.setPosition(352,352);
				break;
			case 4:
				shape.setPosition(352,402);
				break;
			default:
				shape.setPosition(352,452);
				break;
		}
		window.draw(shape);
}

void print_in_second_window(vector<vector <sf::Text> > vec_t, int i){
			int gr = 8;
            for (auto g: vec_t){
...
{	
    ...
    while (window.isOpen()){
        ...
        while (window.pollEvent(event)){
            if (event.type == sf::Event::Closed)
                ...
                if (event.mouseButton.button == sf::Mouse::Left){
					...
					if (start_pr){
						if (sf::IntRect(330, 560, 226, 32).contains(sf::Mouse::getPosition(window))){ 
                            start_pr = false;
							f = true;
							M = my_stoi(s_days);
					...
					if (keycode >= sf::Keyboard::Num0 && keycode <= sf::Keyboard::Num9){
						press_numButton(keycode);
					}
					if (keycode == sf::Keyboard::Up)
						if (cur_input_window > 0)
							cur_input_window--;
					if (keycode == sf::Keyboard::Down)
						if (cur_input_window < 5)
							cur_input_window++;
					input_to_string();
            } 
        }   
        ...
        if (start_pr){
		    ...
		    print_in_second_window(vec_t, i);
        }
        window.display();
    }
    return 0;
